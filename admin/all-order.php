										...
										<div class="row formtype">
											<div class="col-md-3">
												<div class="form-group">
													<label>Order Status &nbsp;</label> <a href="all-order.php"><i style="color:black;" class="fa">&#xf021;</i></a>
											...
											<?php
												if(isset($_GET["filterbtn"])){
													...
													$result = mysqli_query($connect, "SELECT * FROM payment JOIN shipping ON payment.payment_code=shipping.payment_code
																					  WHERE shipping.delivery_status='$shipping_filter' AND delivery_status!=3 ORDER BY payment.payment_date");
											...
											?>
													...
													<td><?php echo $i;?></td>
														...
														<td>
                                                            ...
                                                            {
                                                            ?>
                                                            <i><a href="all-order.php?status&code=<?php echo $row['payment_code'];?>" 
														...
														<ul class="list-inline m-0">
															<li class="list-inline-item">
																<a href="view-order.php?view&code=<?php echo $row['payment_code'];?>"><button class="btn btn-primary btn-sm rounded-0" type="button" title="View"><i class="fa fa-eye"></i></button></a>
															</li>
															<li class="list-inline-item">
																<a href="edit-order.php?edit&code=<?php echo $row['payment_code'];?>"><button class="btn btn-success btn-sm rounded-0" type="button" title="Edit"><i class="fa fa-edit"></i></button></a>
															</li>
														</ul>
														</td>
													</tr>
...
if(isset($_GET['status']))
{
	...
	$cus=mysqli_query($connect,"SELECT * FROM payment INNER JOIN customer ON payment.cus_email=customer.cus_email WHERE payment.payment_code='$code'");
    while($statusrow=mysqli_fetch_assoc($select))
    {
		while($cusrow=mysqli_fetch_assoc($cus))
		{
		...
		if($status=='0')
        {
			...
			$trackid=mysqli_query($connect,"UPDATE payment SET adm_id='$admin' WHERE payment_code='$code'");
			
            $subject = "Your Order is Shipped Out!";
            $message = "Dear ".$cusrow['cus_name'].",\n\nThanks to puchase in 3L Telecommunication!\n\nPlease use below tracking number to track the order. \nOrder Code: ".$code."\nTracking number: ".$uniq."\n\nYou can click the link to track the order.\nhttp://localhost/3L_Telecommunication/index.php\n\nThank you.\n\nRegards,\n3L Telecommunication";
...
<script>
	...
	$('.swal2-confirm').click(function(){
		window.location.href = 'all-order.php';
	});
