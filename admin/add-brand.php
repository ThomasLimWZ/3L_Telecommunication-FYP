							...
							<div class="row">
								<div class="col-lg-12">
									<form method="post" action="" name="myform" enctype="multipart/form-data" autocomplete="off" onsubmit="return validation()">
										<div class="row formtype">
											<div class="col-md-4">
												<div class="form-group">
													<p>Name <span style="color:red;">*</span><input class="form-control" type="text" name="brand_name" value="<?php echo isset($_POST["brand_name"]) ? $_POST["brand_name"] : ''; ?>" required></p>
												</div>
											</div>
											<div class="col-md-4">
												<div class="form-group">
													<label>Brand Image <span style="color:red;">*</span></label>
														<br><input type="file" name="brand_img" required>
										...
										<br>
										<input type="submit" name="savebtn" value="Add Brand" class="btn btn-primary buttonedit ml-2">
										<a href="all-brand.php" class="btn btn-primary buttonedit ml-2">Back to Brand List</a>
									</form>
								</div>
							</div>
						</div>
...
<script>
	...
	var img=document.forms['myform']["brand_img"];
...
if(isset($_POST["savebtn"])){
	...
	$result = mysqli_query($connect,"SELECT * FROM brand WHERE LOWER(brand_name) = '$checkName'");
	$count = mysqli_num_rows($result);
	
	if(isset($_FILES["brand_img"])){
		$pic_name = $_FILES["brand_img"]["name"];
		$pic_size = $_FILES["brand_img"]["size"];
...
?>
					<script>
						swal({title:"This brand is existing in brand list.",
								type:"error"});
					</script>
<?php
				}
					...
					if(in_array($pic_ex_lc, $allowed_exs)){
						...
						$pic_upload_path = 'brand/'.$new_pic_name;
						move_uploaded_file($tmp_name, $pic_upload_path);}
						
					mysqli_query($connect,"INSERT INTO brand(brand_name,brand_image) VAlUES ('$name','$new_pic_name')");
?>
<script>
	swal({
    title: "Record saved.",
	type: "success"
          ...
          $('.swal2-confirm').click(function(){
                window.location.href = 'all-brand.php';
          });
</script>
<?php
				}
